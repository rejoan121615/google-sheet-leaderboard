...
class Leaderboard {
  ...
  getErrorContent(type: ErrorType): { title: string; description: string } {
    if (type === "offline") {
      return {
        title: "No Internet Connection",
        description:
          "Network disconnected. Leaderboard will reconnect automatically when internet returns.",
      };
    }
    ...
    return {
      ...
      description:
        "Unexpected error in leaderboard app. Please refresh the display or restart the device.",
    };
  }
  ...
  createErrorElement(): HTMLElement {
  ...
  ensureLoadingElement(): HTMLElement {
  ...
  ensureErrorElement(): HTMLElement {
  ...
  resolveErrorType(error: unknown): ErrorType {
    if (!navigator.onLine) {
      return "offline";
    }
    ...
    try {
      ...
      if (error instanceof DOMException && error.name === "AbortError") {
        throw new LeaderboardAppError(
          "request-failed",
          "Request timed out while fetching sheet data",
        );
      }

      throw new LeaderboardAppError(
        this.resolveErrorType(error),
        "Request failed while fetching sheet data",
      );
    } finally {
  ...
  async fetchData(): Promise<boolean> {
    ...
    const failureTypes = new Set<ErrorType>();
    ...
    if (hasCachedData) {
      this.showErrorMessage(
        failureTypes.has("offline") ? "offline" : "request-failed",
      );
      return true;
    }
  ...
  showErrorMessage(type: ErrorType) {
