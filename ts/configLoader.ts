...
interface RawConfigRow {
  ...
  "Leaderboard CSV URL": string;
  Sort: string;
  Visibility: string;
}

export async function loadDashboardConfigsFromSheet(
  sheetUrl: string
...
function parseConfigCsv(csvText: string): DashboardConfig[] {
  ...
  const sortIndex = headers.findIndex(
    (h) =>
      h.toLowerCase().includes("sort") ||
      h.toLowerCase().includes("order") ||
      h.toLowerCase().includes("view")
  );
  const visibilityIndex = headers.findIndex(
    (h) =>
  ...
  if (nameIndex === -1 || urlIndex === -1) {
    ...
    throw new Error(
      `Config sheet missing required columns. Found: ${headers.join(", ")}`
    );
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const sort = (values[sortIndex]?.trim().toLowerCase() || "high-to-low") as
      | "high-to-low"
      | "low-to-high";
    const visibility = values[visibilityIndex]?.trim().toLowerCase() || "show";

    console.log(
      `Processing row: name="${name}", visibility="${visibility}", sort="${sort}"`
    );
