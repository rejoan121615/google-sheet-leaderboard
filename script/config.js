...
export const CONFIG_SHEET_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vStRnxoWdWZ1vLSZavLXqcQe4Pq6qve_c8yP9vOY7t05nr0DCPxfLIjl6xnWCrHuaTk7Fy0s7lEknyM/pub?gid=0&single=true&output=csv";
// This will be populated by initializeDashboardConfigs()
export let dashboardConfigs = [];
// Initialize dashboard configurations from Google Sheet
export async function initializeDashboardConfigs() {
    try {
        dashboardConfigs = await loadDashboardConfigsFromSheet(CONFIG_SHEET_URL);
        console.log(`Loaded ${dashboardConfigs.length} dashboard configuration(s) from Google Sheet`);
    }
    catch (error) {
